...
- Tests and fixtures are fully in-package (colocated `*.test.ts` plus `testdata/`, Go style): nothing under `packages/tea-lang` may read paths outside the package tree, so `bun test` passes on a standalone checkout. The third-party Pine corpus is deliberately not vendored (copyright); its conformance gate lives monorepo-side at `scripts/tea-pine-corpus-check.ts`, while `testdata/corpus/` holds original scripts covering the same grammar surface.
