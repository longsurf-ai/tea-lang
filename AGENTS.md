...
- `src/typecheck/` implements the qualifier × value-type checker (native catalog, binder, eager checker with `Info` side tables); it is not wired into `compile()` yet — the check phase barrier lands with the noder.
- `src/testing/` holds cross-pass test DSLs (the shared ERROR-comment harness); pass-specific helpers stay in their pass directory (`syntax/testing.ts`, `typecheck/testing.ts`).
