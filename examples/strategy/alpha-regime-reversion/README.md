...
`ta.rsi` initialization without changing Tea's shared indicator contract. The
local allocation account exists because the shipped Basic portfolio
deliberately supports only all-in/all-out positions.
...
request context. On the 2026-08-14 development run its reported execution
completed in **57.99 s**. Total return ranged from **35.38% to 211.70%**,
maximum drawdown from **27.51% to 43.97%**, and completed round trips from
**22 to 57**. The best-return scenario produced **211.70%**, with **38.36%**
maximum drawdown and **23** completed round trips (`core_allocation=50`, fast
EMA 15, deep RSI 20, 10% emergency stop). The large return and drawdown are
exactly why this result must be treated as a stress fixture rather than an
