...
```sh
bun src/main.ts execute examples/strategy/ai-supertrend-knn/sweep.yaml
```

## Measured result

Validated on 2026-08-14 with the checked-in 3,283-row BTCUSDT daily snapshot:

- 18 bindings / 59,094 evaluated rows
- Tea lowering: 7.28 ms; execution: 40,264.50 ms; reported core total: 40,271.77 ms; process wall time: 40.38 s
...
These are deterministic results for the hash-pinned CSV and the documented local fill model; they are not a claim that Tea reproduces TradingView's proprietary broker emulator bit-for-bit outside the selected source settings.
