// Purpose: Multiple fixed-history bindings run in caller order with isolated state.
...
import {executeFixedHistory} from './fixed-history';
...
describe('executeFixedHistory', () => {
  test('runs ordinary bindings in caller order with isolated state and sinks', async () => {
    ...
    const results = await executeFixedHistory(module(), [
      {
        params: {scale: 1},
  ...
  test('accepts an empty binding list', async () => {
    expect(await executeFixedHistory(module(), [])).toEqual([]);
  });
});
