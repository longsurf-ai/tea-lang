...
import {funcsOf, namesOf, seriesInputsOf} from '../ir/visit';
import type {
  DepthSpec,
  FrameLayout,
  ModuleManifest,
...
class Generator {
  ...
  private readonly usedHelpers = new Set<HelperName>();
  ...
  generate(): string {
    ...
    for (const name of [...this.usedHelpers].sort()) {
    ...
    for (const [key, lines] of initThunks) {
      ...
      out.push(...indent(indent(indent(lines))));
      out.push('    },');
    }
    out.push('  },');
    out.push('  funcs: {');
    for (const [fid, lines] of funcBodies) {
      ...
      out.push(...indent(indent(lines.slice(1))));
    }
    out.push('  },');
    out.push('  main(rt, fr) {', ...indent(indent(mainLines)), '  },');
    out.push('};');
    out.push('return M;');
    return `${out.join('\n')}\n`;
  }
  ...
  private buildManifest(): ModuleManifest {
    ...
    return {series, params, outputs, frames};
  }
}
