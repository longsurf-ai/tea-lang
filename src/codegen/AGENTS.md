...
- The CPU artifact is TypeScript ESM exporting one ABI-11 `Module<Context>`.
...
- Persistent declarations remain lexical `InitName` statements, lowered to lazy
  `Series.init(() => value)`. Context owns the transaction and commits only
...
- Arithmetic and intrinsics call runtime methods/functions, including integer
  ...
  remain lazy; conditional expressions capture all three operands eagerly.
...
- One output schema owns names, payload types, write mode, and kind. Set fields
  precede append fields in a common output-ID space. Generated destinations use
  `.set({channels})` or `.append(payload)` and capture values at that point.
  Display labels never become declaration identities. Binding and channel
  arguments evaluate in source order before canonical output assembly.
