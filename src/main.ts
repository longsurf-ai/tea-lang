...
async function dispatch(command: CliCommand): Promise<CliResult> {
  switch (command.kind) {
    ...
    case 'build': {
      const result = compile([command.file]);
      if (!result.ok) {
      ...
      if (command.out === undefined) {
        ...
        writeFileSync(command.out, result.js);
      }
      return {ok: true};
    }
    ...
    case 'parse': {
      ...
      const wantAst = command.ast || (!command.tokens && !command.ir);

      if (command.tokens) {
        const source = readFileSync(command.file, 'utf8');
        const tokens = tokenize(
          newFileBase(command.file),
          source,
          (pos, message) => errors.errorAt(pos, message),
        );
        console.log(dumpTokens(tokens));
      }
      if (wantAst) console.log(dumpFile(parseFile(command.file, errors)));
      if (command.ir) {
        const program = compileToProgram([command.file], errors);
