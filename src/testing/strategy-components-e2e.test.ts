...
describe('Tea strategy components end to end', () => {
  test('keeps two configured strategy values isolated through scripted phases', async () => {
    const source = [
      ...
      'import portfolio',
      'import trade',
      'var primary = trade.net(broker.basic(), portfolio.basic(100.0))',
      'var secondary = trade.net(broker.basic(), portfolio.basic(100.0))',
    ...
    expect(funcsOf(program).map(func => func.name)).toEqual(
      expect.arrayContaining([
        ...
        'NetTrade<BrokerEmulator, NetPortfolio>.close',
  ...
  test('retains an entry submitted after begin until a later bar', async () => {
    const source = [
      ...
      'import portfolio',
      'import trade',
      'var strat = trade.net(broker.basic(), portfolio.basic(100.0))',
  ...
  test('fills on the next open and applies slippage, fees, and round-trip accounting', async () => {
    const source = [
      ...
      'import portfolio',
      'import trade',
      'var strat = trade.net(broker.basic(0.1, 0.1), portfolio.basic(121.0))',
      'strat.begin(open, bar_index)',
      'if bar_index == 0',
      '    strat.entry("Long", trade.Direction.long)',
      'if bar_index == 1',
  ...
  test('distinguishes resting buy-stop gap, intrabar, and missed fills', async () => {
    const source = [
      ...
      'import portfolio',
      'import trade',
      'var gap = trade.net(broker.new(), portfolio.new(initialCash = 100.0))',
      'var intrabar = trade.net(broker.new(), portfolio.new(initialCash = 100.0))',
      'var missed = trade.net(broker.new(), portfolio.new(initialCash = 100.0))',
  ...
  test('replaces a resting buy stop and cancels its contingent bracket atomically', async () => {
    const source = [
      ...
      'import portfolio',
      'import trade',
      'var strat = trade.net(broker.new(), portfolio.new(initialCash = 100.0))',
      'strat.begin_bar(open, high, low, bar_index)',
      'int cancelled = 0',
      'if bar_index == 0',
  ...
  test('installs a fill-derived bracket before replaying the same bar exit path', async () => {
    const source = [
      ...
      '    portfolio = portfolio.new(initialCash = 100.0, marginLong = 100.0)',
      ')',
      'primary = strat.begin_primary(open, high, low, bar_index)',
      'if bar_index == 0',
      '    strat.entry("Long", trade.Direction.long, qty = 1.0)',
