// Purpose: Human-readable indexed table; TraceSink remains the machine format.
...
export class TableSink implements OutputSink {
  ...
  publish(publication: Datum): void {
    let entry = this.byRow.get(publication.index);
    if (entry === undefined) {
      entry = {provisional: publication.provisional, values: new Map()};
      this.byRow.set(publication.index, entry);
    }
